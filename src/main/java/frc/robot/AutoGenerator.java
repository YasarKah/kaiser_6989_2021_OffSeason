...
public class AutoGenerator {
  ...
  private Command makeCenter3BallAuto() {
    ...
    var autoVoltageConstraint = new DifferentialDriveVoltageConstraint(
        ...
        Constants.DriveConstants.kDrive_Kinematics, 10);

    TrajectoryConfig config = new TrajectoryConfig(DriveConstants.kMax_Speed_Meters_Per_Second,
        DriveConstants.kMax_Acceleration_Meters_Per_Second_Squared).setKinematics(DriveConstants.kDrive_Kinematics)
            .addConstraint(autoVoltageConstraint).setStartVelocity(15).setEndVelocity(0).setReversed(false);
    ...
    return m_drive.createCommandForTrajectory(trajectory);
        /*.alongWith(new InstantCommand(m_shooter::shoot_Start, m_shooter))
...
/*
    return new InstantCommand(m_shooter::shoot_Start, m_shooter)
    .andThen(new WaitCommand(3))
    .andThen(m_shooter::shoot_Stop, m_shooter);*/
  }
  private Command Yasar(){
    System.out.println("Haha");
    return new InstantCommand(m_intake::open_Intake, m_intake)
    .andThen(new WaitCommand(2))
    .andThen(m_intake::close_Intake, m_intake)
    .andThen(new PrintCommand("SaGardaş"));
  }
  private Command steal_trench() {
    ...
    TrajectoryConfig config = new TrajectoryConfig(DriveConstants.kMax_Speed_Meters_Per_Second,
        DriveConstants.kMax_Acceleration_Meters_Per_Second_Squared).setKinematics(DriveConstants.kDrive_Kinematics)
            .addConstraint(autoVoltageConstraint).setStartVelocity(0).setEndVelocity(0).setReversed(false)
            .addConstraint(new CentripetalAccelerationConstraint(0.7));

    TrajectoryConfig r_config = new TrajectoryConfig(DriveConstants.kMax_Speed_Meters_Per_Second,
        DriveConstants.kMax_Acceleration_Meters_Per_Second_Squared).setKinematics(DriveConstants.kDrive_Kinematics)
            .addConstraint(autoVoltageConstraint).setStartVelocity(0).setEndVelocity(0).setReversed(true)
            .addConstraint(new CentripetalAccelerationConstraint(0.7));

    var trajectory_straight = TrajectoryGenerator.generateTrajectory(new Pose2d(0, 0, new Rotation2d(0)),
        Collections.emptyList(), new Pose2d(3.3, 0, new Rotation2d(0)), config);
    ...
    var trajectory_curved_final = TrajectoryGenerator.generateTrajectory(new Pose2d(-2, 0, new Rotation2d(0)),
        Collections.emptyList(), new Pose2d(-2.3, -4.59, new Rotation2d(-3.12)), config);

    var trajectory_1 = m_drive.createCommandForTrajectory(trajectory_straight)
        .alongWith(new InstantCommand(m_intake::start_Intake, m_intake));
    var trajectory_2 = m_drive.createCommandForTrajectory(trajectory_straight_r);
    var trajectory_3 = m_drive.createCommandForTrajectory(trajectory_curved_final)
        .alongWith(new ShootCommand(m_shooter));

        //return new InstantCommand(m_intake::open_Intake, m_intake).withTimeout(2).andThen(new InstantCommand(m_intake::close_Intake, m_intake)).withTimeout(2).andThen(new InstantCommand(m_intake::open_Intake, m_intake)).withTimeout(2).andThen(new InstantCommand(m_intake::close_Intake, m_intake));
    return new InstantCommand(m_intake::close_Intake, m_intake).andThen(trajectory_1).withTimeout(3)
        .andThen(new InstantCommand(m_intake::stop_Intake, m_intake)).andThen(trajectory_2).andThen(trajectory_3)
        .andThen(m_indexer::start_Indexer_auto, m_indexer).andThen(m_intake::start_Intake, m_intake);
